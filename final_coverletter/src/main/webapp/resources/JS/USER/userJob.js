...
function paging(curPage) {
	location.href = "USER_userJobList.do?curPage=" + curPage;
}

/*---------- 상세글보기 : boardDetail func ----------*/
// function boardDetail(boardseq) {
// var curPage = $('.paging-focus').text();
// location.href='BOARD_boardDetail.do?boardseq=' + boardseq + '&curPage=' +
// curPage;
// }
/*---------- 검 색 : search func ----------*/
function search() {
	...
	// controller
	location.href = 'USER_userJobList.do?category=' + category + '&keyword='
			+ keyword;
}
...
function deleteSubmit() {
	...
	if ($("input[name=chk]:checked").length == 0) {
		...
		$('.modal-body').text('한개 이상 체크 해주세요. ^^*');
		$('#myModal').modal('show');
		return false;
	} else {
		var form = $('form');
		form.action = 'USER_userJobList.do';
		form.submit();
