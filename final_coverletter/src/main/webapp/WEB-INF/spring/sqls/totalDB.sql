...
SELECT COUNT(*)	FROM COVERLETTER WHERE GROUPSEQ = 1;

SELECT * FROM COVERLETTER WHERE CVCATEGORY = '자소서';
SELECT COUNT(*) FROM COVERLETTER WHERE CVCATEGORY = '자소서';
...
INSERT INTO COVERLETTER VALUES(TOTAL_SEQ.NEXTVAL, 'UESR@GMAIL.COMM', '포폴', PORTFOLIO_SEQ.NEXTVAL, NULL, '제목:23', NULL, NULL, SYSDATE, '파일')

SELECT * FROM COVERLETTER

DELETE FROM COVERLETTER WHERE CVCATEGORY = '자소서' AND GROUPSEQ IN 2;
...
SELECT COUNT(*)
		FROM JOBCALENDAR
		WHERE JOINEMAIL = 'USER@GMAIL.COM';

SELECT B.*
		FROM (
		SELECT A.*, ROW_NUMBER() OVER(ORDER BY JOBCALENDARSEQ DESC) AS RNUM
		FROM JOBCALENDAR A
		WHERE JOINEMAIL = 'USER@GMAIL.COM'
		) B
		WHERE RNUM BETWEEN 1 + 1 AND 10
		ORDER BY JOBCALENDARSEQ DESC	
	
SELECT * FROM JOBCALENDAR WHERE JOINEMAIL = 'USER@GMAIL.COM';

INSERT INTO JOBCALENDAR VALUES(JOBCALENDAR_SEQ.NEXTVAL,'USER@GMAIL.COM',10,'회사이름','채용제목: 직원 안구함구함','상시채용');
INSERT INTO JOBCALENDAR VALUES(JOBCALENDAR_SEQ.NEXTVAL,'USER@GMAIL.COM',11,'회사이름의','.채용제목: 직원 안구함구함','2020-03-21');
INSERT INTO JOBCALENDAR VALUES(JOBCALENDAR_SEQ.NEXTVAL,'USER@GMAIL.COM',12,'회사이름의난','..채용제목: 직원 안구함구함','2020-03-22');
