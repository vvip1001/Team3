...
(COVERLETTER_SEQ.NEXTVAL, 'cv@email.net', 'CV', '지원자의 성장과정...?', '운토티 제출용', '저는 우량아로 태어나..', '대통령은 헌법과 법률이 정하는 바에 의하여 국군을 통수한다. 공공필요에 의한 재산권의 수용·사용 또는 제한 및 그에 대한 보상은 법률로써 하되, 정당한 보상을 지급하여야 한다. 대통령후보자가 1인일 때에는 그 득표수가 선거권자 총수의 3분의 1 이상이 아니면 대통령으로 당선될 수 없다. 법관이 중대한 심신상의 장해로 직무를 수행할 수 없을 때에는 법률이 정하는 바에 의하여 퇴직하게 할 수 있다.', SYSDATE, 'aaa');
